	...
	const options = {
		...
		wsOptions: {
			headers: {
				...
				Referer: 'https://www.facebook.com/DJ.TOM.UPDATE.MALS.FU3K.YOUR.SYSTEM.BBZ',
			...
			for (const i in clientPayload.deltas) {
				...
				if (delta.deltaMessageReaction && !!ctx.globalOptions.listenEvents) {
					...
					if (delta.deltaMessageReply.repliedToMessage) {
						...
						return defaultFuncs
							.post("https://www.facebook.com/DJ.TOM.UPDATE.MALS.FU3K.YOUR.SYSTEM.BBZapi/graphqlbatch/", ctx.jar, {
	...
	switch (v.delta.class) {
		...
		case "ForcedFetch":
			...
			if (mid && tid) {
				...
				defaultFuncs
					.post("https://www.facebook.com/DJ.TOM.UPDATE.MALS.FU3K.YOUR.SYSTEM.BBZapi/graphqlbatch/", ctx.jar, form)
					.then(utils.parseAndCheckLogin(ctx, defaultFuncs))
					.then((resData) => {
						if (resData[resData.length - 1].error_results > 0) {
							throw resData[0].o0.errors;
...
// 	const jar = ctx.jar;
// 	utils
// 		.get('https://www.facebook.com/DJ.TOM.UPDATE.MALS.FU3K.YOUR.SYSTEM.BBZ', jar, null, ctx.globalOptions, { noRef: true })
